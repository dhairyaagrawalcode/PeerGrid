...
returns uuid language plpgsql security definer set search_path = '' as $$
declare current_user_id uuid := auth.uid(); clean_member_ids uuid[]; conversation_id uuid;
begin
  if not public.is_verified_student() or char_length(trim(candidate_title)) not between 2 and 80 then raise exception 'INVALID_GROUP'; end if;
  select array_agg(distinct member_id order by member_id) into clean_member_ids
  from unnest(candidate_member_ids) member_id where member_id is not null and member_id <> current_user_id;
  ...
  insert into public.conversations(kind, title, created_by) values ('group', trim(candidate_title), current_user_id) returning id into conversation_id;
  insert into public.conversation_members(conversation_id, profile_id, role) values (conversation_id, current_user_id, 'owner');
  insert into public.conversation_members(conversation_id, profile_id, role) select conversation_id, member_id, 'member' from unnest(clean_member_ids) member_id;
  insert into public.conversation_read_state(conversation_id, profile_id)
    select member.conversation_id, member.profile_id from public.conversation_members member where member.conversation_id = conversation_id;
  insert into public.notifications(recipient_id, actor_id, type, conversation_id)
    select member_id, current_user_id, 'added_to_group', conversation_id from unnest(clean_member_ids) member_id on conflict do nothing;
  return conversation_id;
end;
$$;
