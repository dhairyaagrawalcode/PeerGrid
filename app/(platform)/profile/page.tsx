...
import {
  ProfileConfirmationsSkeleton,
  ProfilePostsSkeleton,
  ProfileProofsSkeleton,
...
export default async function ProfilePage({
  ...
  const proofPage =
    Number.isSafeInteger(rawProofPage) && rawProofPage > 0 ? rawProofPage : 0;
  ...
  return (
    <div className="app-page">
      {params.confirmation === "error" && (
        <p
          className="mx-auto mb-5 max-w-[920px] rounded-lg border border-danger/30 bg-danger/10 px-4 py-3 text-sm text-danger"
          role="alert"
        >
          Could not update this collaboration confirmation. Apply the latest
      ...
      <ProfileView
        ...
        profile={profile}
        pendingContent={
          <Suspense fallback={<ProfileConfirmationsSkeleton />}>
            <ProfilePendingSection />
          </Suspense>
        }
        proofsContent={
          <Suspense
            key={`profile-proofs:${proofPage}`}
            fallback={<ProfileProofsSkeleton />}
          >
            <ProfileProofSection
              profile={profile}
              page={proofPage}
        ...
        postsContent={
          <Suspense
            key={`profile-posts:${page}`}
            fallback={<ProfilePostsSkeleton />}
          >
            <ProfilePostSection
              profile={profile}
              page={page}
