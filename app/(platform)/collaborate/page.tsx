...
import { getCampuses, getCollaborations, getConnections } from "@/app/lib/data";

export default async function CollaboratePage() {
  const { supabase, user } = await requireStudent();
  const [campuses, posts, connections] = await Promise.all([getCampuses(supabase), getCollaborations(supabase), getConnections(supabase, user.id)]);
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
      <div className="min-w-0 xl:order-1">
        ...
        <div className="mt-6 space-y-4">{posts.length ? posts.map((post) => <CollaborationCard key={post.id} post={post} currentId={user.id} connection={connectionMap.get(post.author_id)} />) : <EmptyState icon={<FiUsers size={21} />} title="No collaboration posts yet" copy="Share the first open call with students across NST." />}</div>
      </div>
