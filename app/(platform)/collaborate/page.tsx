  ...
  return (
    <div className="app-page grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
      ...
      <div className="min-w-0 xl:order-1">
        <div className="mobile-hide"><p className="eyebrow">Collaboration board</p><h1 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">Build with people, not alone</h1><p className="mt-2 text-sm text-muted">Find an open project, connect with its creator in DM, and build together.</p></div>
        ...
        <div className="collaboration-list mt-6 space-y-4">{visiblePosts.length ? visiblePosts.map((post) => <CollaborationCard campuses={campuses} key={post.id} post={post} currentId={user.id} />) : <EmptyState icon={<FiUsers size={21} />} title="No collaboration posts yet" copy="Share the first open call with students across NST." />}</div>
        <PageNavigation hasMore={hasMore} page={page} path="/collaborate" />
      </div>
      <aside className="mobile-hide xl:order-2">
