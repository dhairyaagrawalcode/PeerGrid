...
type Search = { q?: string; campus?: string; skill?: string; interest?: string };
...
export default async function DiscoverPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  return (
    <div>
      ...
      <form className="surface mt-6 grid gap-3 p-4 sm:grid-cols-2 lg:grid-cols-4" method="get">
        <div className="relative sm:col-span-2 lg:col-span-4"><FiSearch className="absolute left-3.5 top-3.5 text-muted" /><input className="field !pl-10" name="q" defaultValue={filters.q} placeholder="Name, goal, skill, or interest" /></div>
      ...
      <div className="mt-6 flex items-center justify-between"><p className="text-sm font-semibold">{filtered.length} student{filtered.length === 1 ? "" : "s"}</p>{Object.values(filters).some(Boolean) && <a className="text-xs font-bold text-muted hover:text-font" href="/discover">Clear filters</a>}</div>
      {filtered.length ? <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{filtered.map((student) => <StudentCard key={student.id} student={student} currentId={user.id} connection={connectionMap.get(student.id)} />)}</div> : <div className="mt-4"><EmptyState icon={<FiUsers size={21} />} title="No students match yet" copy="Try a broader search, remove one filter, or check back as more verified students join." /></div>}
    </div>
  );
}
