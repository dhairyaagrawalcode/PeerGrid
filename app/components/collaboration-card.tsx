  ...
  return (
    <article className="surface p-5 sm:p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className={`chip ${post.status === "open" ? "!border-secondary/15 !bg-secondary/8 !text-secondary" : "!text-muted"}`}>{post.status}</span>
      </div>
      <h2 className="mt-5 text-lg font-bold leading-snug sm:text-xl">{post.title}</h2>
      <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#c7cbd5]">{post.description}</p>
