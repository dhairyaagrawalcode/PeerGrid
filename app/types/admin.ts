...
export type AdminUser = { id: string; email: string; full_name: string | null; username: string | null; avatar_url: string | null; campus: string | null; graduation_year: number | null; created_at: string; last_active_at: string | null; account_status: AccountStatus; approval_status: string | null; posts: number; collaborations: number; followers: number; following: number; post_reports: number; issues: number; is_admin: boolean };
export type AdminOverviewSnapshot = AdminOverview & { profile_count: number; accounts_without_profile: number; unresolved_reports: number; suspended_accounts: number; recent_admin_activity: AuditEvent[]; refreshed_at: string };
...
export type AdminUserFilterOptions = { campuses: { id: string; name: string }[]; years: number[] };
